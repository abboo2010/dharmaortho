...
ready(function () {
  ...
  var videoDataEl = document.getElementById('video-data');
  ...
  if (videoDataEl && videoLightbox) {
    ...
    function stepVideo(delta) {
      vIndex = (vIndex + delta + videos.length) % videos.length;
      renderVideoLightbox();
    }

    document.querySelectorAll('.video-card').forEach(function (card) {
      card.addEventListener('click', function () {
        openVideoLightbox(card.getAttribute('data-video'));
      });
    });
    videoLightbox.querySelectorAll('[data-close]').forEach(function (el) {
      el.addEventListener('click', closeVideoLightbox);
    });
    videoLightbox.querySelector('.lightbox-prev').addEventListener('click', function () { stepVideo(-1); });
    videoLightbox.querySelector('.lightbox-next').addEventListener('click', function () { stepVideo(1); });
    document.addEventListener('keydown', function (e) {
